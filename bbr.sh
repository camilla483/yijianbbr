...
#
# youtube频道: https://www.youtube.com/@user-wr7rz2jq4z
#
...
sysctl -p >/dev/null 2>&1
echo "Only TCP BBR has already been disabled..."
sleep 3
...
echo    "#####################################################################################"
echo -e "#                         ${RED}BBR一键安装脚本${PLAIN}                                             #"
echo -e "# ${GREEN}支持${PLAIN}: CentOS 6+、Ubuntu、Debian                                                     #"
...
[[ -z $1 ]] && action=menu
case "$action" in
    menu|start_bbr|stop_bbr)
        ${action}
        ;;
    *)
        echo " 参数错误"
        echo " 用法: `basename $0` [menu|start_bbr|stop_bbr]"
        ;;
esac